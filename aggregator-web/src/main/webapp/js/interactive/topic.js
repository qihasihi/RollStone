...
//                        h+='<a href="javascript:;" onclick="if(confirm(\'您确定要删除此论题吗?\\n\\n提示：删除该论题后可在回收站里恢复!\')){updateTopicStatus('+itm.topicid+',3);}" class="ico04" title="删除"></a>';
                    ...
                    h+='<a  class="font-darkblue" href="tptopic?m=viewTopic&topicid='+itm.topicid+'">'+itm.topictitle+'</a> </p><!-- ico49a -->';
                    h+='</div>';
    ...
    $.ajax({
        ...
        error:function(){
            ...
            if(rps.type=="error"){
                ...
                if(val!=3){
                    if(val==1){
                        $("#a_status_"+id).removeClass("ico53");
                        $("#a_status_"+id).addClass("ico54");
                        $("#a_status_"+id).attr({"alt":"关闭论题","title":"关闭论题"});
                    }else if(val==2){
                        $("#a_status_"+id).attr({"alt":"开放论题","title":"开放论题"});
                        $("#a_status_"+id).removeClass("ico54");
                        $("#a_status_"+id).addClass("ico53");
                    }
