		...
		error:function(){
	 	...
	 	success:function(json){ 
		 	...
		 	if(questype>0&&boo){
                ...
                $("#tb_ques").show();
		 		$("input[name='rdo_ques_type']").filter(function(){return this.value==questype}).attr('checked',true);
		  		$("input[name='rdo_ques_type']").filter(function(){return this.value!=questype}).attr('disabled',true);
                changeQuesType(questype,questionid);
            }
	 	}   
	 });  
...
function queryInteraction(courseid,trobj,taskvalueid,taskstatus){
	...
	var cls=$("input[type='checkbox']").filter(function(){return this.id.indexOf("ckb_")!=-1&&this.checked==true});
	var clsid='';  
...
function doSubManageTask(taskid){ 
    ...
    var clsarr=$("input[name='ck_cls']:checked");/*.filter(function(){
...
/*	if(taskname.val().Trim().length<1){
	...
	//课后作业
	var quesoptionArray=$("textarea").filter(function(){return this.id.indexOf("ques_option_")!=-1});   
	if(tasktype.val()=="3"){
        if(quesid.length<1){
            alert('请选择试题!');
            return;
