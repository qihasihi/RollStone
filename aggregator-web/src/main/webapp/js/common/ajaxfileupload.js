jQuery.extend( {
		...
		createUploadForm : function(id, fileElementId, formName) {
			// create form
			...
			var flag = false;
			// 在同一界面存在多个功能的上传时，无法获取File
			if (typeof (formName) == "undefined") {
				if (typeof (oldElement.length) != "undefined") {
					for ( var z = 0; z < oldElement.length; z++) {
						if (oldElement[z].value!= "") {
							flag = true;
							break;
						}
					}
					if(!flag)
						oldElement =  jQuery("form[name='"+formName+"'] input[name='"+fileElementId+"']");
				} else {
					if (typeof (formName) == "undefined")
						oldElement =  jQuery("form[name='"+formName+"'] input[name='"+fileElementId+"']");
					else
			...
			if (typeof (oldElement) != "undefined") {
				if (typeof (oldElement.length) != "undefined") {
					for ( var z = 0; z < oldElement.length; z++) {
						var newElement = jQuery(oldElement[z]).clone();
						jQuery(oldElement[z]).before(newElement);
					}
				} else {
					if (oldElement.value != "") {
						var newElement = jQuery(oldElement).clone();
		...
		ajaxFileUpload : function(s) {
			...
			var form = jQuery.createUploadForm(id, s.fileElementId, s.formName);
