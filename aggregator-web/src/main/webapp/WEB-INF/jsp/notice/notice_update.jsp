...
%>
  <head>
	...
	var istitleLink='${ntc.titlelink}';
	var beginTime='${ntc.begintime}';
	var endTime='${ntc.endtime}';
	$(function(){	
		typeSelect('tdNoticeUsefullife');
		$("#type").val("${ntc.noticetype}");
		...
		var roles = '${ntc.noticerole}';
		var rolelist=roles.split("|");
			var objRoleId=document.getElementsByName("roleId");
			if(objRoleId!=null){
		...
		if('${ntc.noticegrade}'!=null||'${ntc.noticegrade}'!=""){
			var grades = '${ntc.noticegrade}';
			var gradelist = grades.split("|"); 
			var grade = document.getElementsByName("ck_grade");
			if(grade!=null){
			...
			<a href="loginJumpFile.jsp">返回首页</a>
				<table style="font-size: 13px">
					...
					<tr>
						...
						<th>						
							<input type="checkbox" id="isTop" name="isTop" value="0"
								checked="${ntc.istop eq 0?" true":"false" }" />
							是否置顶&nbsp;&nbsp;						
						</th>
					</tr>
					...
					<tr>
						...
						<td colspan="3">
							<input type="text" name="title"  id="title" style="width: 650px" value="${ntc.noticetitle }"
								maxlength="50" />
						</td>
					</tr>
					<tr>
						<th>标题链接:</th>
						<td colspan="3" align="left"><input type="checkbox" onclick="isTitleLinkEditor(this)" ${ntc.titlelink=='0'?"checked='checked'":"" } id="titleLink" name="titleLink" value="0"/></td>
					</tr>
					<tr>
						<th valign="top">
							内 &nbsp;&nbsp;容:
						...
						<td colspan="3" id="tdContent">
							 <textarea rows="15" name="content" style="height:300px" id="content" cols="70">${ntc.noticecontent}</textarea>
					</tr>
					<tr>
						<th>
							发送对象:
   					...
   					<td colspan="3">
   						...
   						<c:if test="${!empty gradeList }">
   							<c:forEach var="g" items="${gradeList }" varStatus="status">
   								<input type="checkbox"  value="${g.gradevalue }" id="ck_grade" name="ck_grade"/>${g.gradename }
   								<c:if test="${(status.index+1)%3==0}"><br/></c:if>
   							</c:forEach>
   						</c:if>
   					</td>
   				</tr>
   				<input type="hidden" name="ref" id="ref" value="${ntc.ref }"/>
   				
   			</table>
   			<input type="button" onclick="onSubmitdoUpd()" value="确定" />
							&nbsp;&nbsp;&nbsp;
