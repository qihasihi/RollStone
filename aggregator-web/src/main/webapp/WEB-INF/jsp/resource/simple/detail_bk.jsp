...
//
//                        html+='<a href="javascript:;" style="cursor:default"><span class="ico41" title="赞"></span>';
//
//                        html+='<b><em id=\''+itm.commentid+'_support\'>'+itm.support+'</em></b></a>';
//
//                        html+= '<a  href="javascript:;"  style="cursor:default"><span class="ico72" title="踩">'
...
//        }
        ...
        function doSimpleSearch(){
            ...
            if(searchType==1){
                var url="simpleRes?m=resList&srhValue="+$("#searchValue").val().Trim();
                url=encodeURI(encodeURI(url));
                window.open(url);
            }
//            if(searchType==2){
//                var url="simpleRes?m=toteacherreslist&srhValue="+$("#searchValue").val();
//                url=encodeURI(encodeURI(url));
//                window.open(url);
//            }
            if(searchType==3){
                var url="simpleRes?m=courseList&srhValue="+$("#searchValue").val();
                url=encodeURI(encodeURI(url));
  ...
  <p class="t_r "><input name="searchValue" id="searchValue" type="text" class="w320"/>
      ...
      &nbsp;<a class="an_search" href="javascript:doSimpleSearch();" title="搜索"></a></p>
  <!--图片-->
  <div class="zyxt_zyxq">
  <c:if test="${fileType=='video'||fileType=='mp3'||fileType=='swf'}">
      ...
      <div class="zyxt_zyxqR">
          ...
          <p>发&nbsp;布&nbsp;者：${empty resObj.realname?"北京四中网校":resObj.realname}</p>
          <p class="info">资源简介
              <c:if test="${sessionScope.CURRENT_USER.userid==resObj.userid}">
  ...
  <c:if test="${fileType=='doc'||fileType=='other'}">
      ...
      <p><span class="w220"><span id="sp_grade">${resObj.gradename}</span>&nbsp;
          ...
          发布者：${empty resObj.realname?"北京四中网校":resObj.realname}
  ...
  <%/*  <p class="f_right">
          ...
          <p class="ico">
              <c:if test="${ispraise==2}">
		             <span id="praise_status">
		                <!--<a href="javascript:;" onclick="operatePraise('${resObj.resid }')"><span class="ico41" title="赞"></span><span>${resObj.praisenum}</span></a>-->
                         <a href="javascript:;"  style="cursor: default"><span class="ico41" title="赞"></span><span>${resObj.praisenum}</span></a>
                    </span>
              </c:if>
              ...
              <c:if test="${isrecomend==2}">
		         ...
		         <!--<a href="javascript:;" onclick="operateRecommend('${resObj.resid }')"><span class="ico73" title="推荐"></span><span>${resObj.recomendnum}</span></a>-->
                     <a href="javascript:;" style="cursor: default"><span class="ico73" title="推荐"></span><span>${resObj.recomendnum}</span></a>
                 </span>
              </c:if>
		    ...
		    <c:if test="${isstore==2}">
                ...
                <a href="javascript:;" style="cursor: default"><span class="ico58a" title="收藏"></span><span>${resObj.storenum}</span></a>
            </c:if>
		     <c:if test="${isstore==1}">
                 ...
                 <a href="javascript:;" style="cursor: default"><span class="ico58b" title="取消收藏"></span><span>${resObj.storenum}</span></a>
             </c:if>
		    </span>
      ...
      <%--<div class="zyxt_zyxq_pl public_input">--%>
          ...
          <%--</ul>--%>
          <%--&lt;%&ndash;<div class="fabiao">&ndash;%&gt;--%>
              <%--&lt;%&ndash;<textarea name="textarea" id="commentcontext" name="commentcontext" ></textarea>&ndash;%&gt;--%>
              <%--&lt;%&ndash;<p class="t_r">最多可以输入200个字!&nbsp;&nbsp;&nbsp;&nbsp;&ndash;%&gt;--%>
                  <%--&lt;%&ndash;<input name="input" type="checkbox"  value="1" id="anonymous" name="anonymous">&ndash;%&gt;--%>
                  <%--&lt;%&ndash;<span class="font-black">匿名</span>&nbsp;&nbsp;<a href="javascript:;" onclick="addResourceComment();" class="an_small">发&nbsp;表</a></p>&ndash;%&gt;--%>
          <%--&lt;%&ndash;</div>&ndash;%&gt;--%>
