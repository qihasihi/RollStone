            ...
            <tr>
                ...
                <td>
                    <div class="w430">
                        ...
                        <div id="all_ct">
                            <p><span>全部班级:</span><input class="w140" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly" id="all_classtime"  name="all_classtime" type="text" /></p>
                        </div>
                        <div id="list_ct" style="display:none">
                            <c:forEach var="cl" items="${ cuList}">
                                <p id="p_${cl.classid}"  style="display:none"><span>${cl.classname}:</span>&nbsp;
                                    <input class="public_input w220" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly"   id="${cl.classid}_classtime"  name="classtime" type="text" /></p>
                            </c:forEach>
                            <c:forEach var="ts" items="${ tsList}">
                                <c:if test="${ts.CLASS_TYPE==1}">
                                    <p id="p_ts_${ts.CLASS_ID}" style="display:none"><span>${ts.CLASSNAME}:</span>&nbsp;
                                        <input class="public_input w220" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly"   id="${ts.CLASS_ID}_ts_classtime"  name="ts_classtime" type="text" /></p>
                                </c:if>
                            </c:forEach>
                            <c:forEach var="tvc" items="${ tvcList}">
                                <p id="p_v_${tvc.virtualclassid}"  style="display:none"><span>${tvc.virtualclassname}:</span>&nbsp;
                                    <input class="public_input w220" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly"   id="${tvc.virtualclassid}_v_classtime"  name="v_classtime" type="text" /></p>
                            </c:forEach>
                            <c:forEach var="ts" items="${ tsList}">
                                <c:if test="${ts.CLASS_TYPE==2}">
                                    <p id="p_v_${ts.CLASS_ID}"><span>${ts.CLASSNAME}:</span>&nbsp;
                                        <input class="public_input w220" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly"   id="${ts.CLASS_ID}_v_classtime"  name="v_classtime" type="text" /></p>
                                </c:if>
                            </c:forEach>
                        </div>
                    </div>
