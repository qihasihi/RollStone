...
<script type="text/javascript">
    ...
    <c:if test="${c.ishas ne '1'}">
    $("#ck_group_${c.groupid}").attr("disabled",false);
    </c:if>
    </c:forEach>
    </c:if>
    ...
    <c:if test="${tg.taskstatus ne '3'}">
    var objArray=$("input[name='ck_cls']:not([data-bind='1'])").filter(function(){return this.value=="${tg.usertypeid}"});
    if(objArray.length>0){
        $.each(objArray,function(ix,im){
            $(im).attr("disabled",false);
            $("#p_group_${tg.usertypeid} input").attr("disabled",false);
            $("#p_ck_${tg.usertypeid}").attr("disabled",false);
        });
    }
    </c:if>
