		...
		function loadDeptByType(){
			<c:if test="${!empty deptType}">
		    	<c:forEach items="${deptType}" var="dt">
		    		var tmpStr="p${dt.deptid} = new PageControl({";
		    			tmpStr+="post_url:'deptuser?m=indexGetListPage&depttypeid=${dt.deptid}',";
		    			tmpStr+="page_id:'page${dt.deptid}',";
		    			tmpStr+="page_control_name:'p${dt.deptid}',";
		    			tmpStr+="post_form:document.page${dt.deptid}form,";
		    			tmpStr+="gender_address_id:'page${dt.deptid}address',";
		    			tmpStr+="http_operate_handler:deptListReturn,";
		    			tmpStr+="return_type:'json',page_no:1,page_size:9,rectotal:0,pagetotal:1,";
		    			tmpStr+="operate_id:'ul_dept_${dt.deptid}'})";
					eval("("+tmpStr+")");
					pageGo("p${dt.deptid}");
				</c:forEach>
			</c:if>
