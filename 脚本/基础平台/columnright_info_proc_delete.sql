...
DROP PROCEDURE IF EXISTS `columnright_info_proc_delete`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `columnright_info_proc_delete`(
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql="delete from columnright_info where 1=1";
	...
	IF p_columnright_name IS NOT NULL THEN
		SET tmp_sql=CONCAT(tmp_sql," and COLUMNRIGHT_NAME='",p_columnright_name,"'");
	END IF;
