...
CREATE DEFINER=`mytest`@`%` PROCEDURE `generator_for_admin_performance`()
BEGIN
	...
	DECLARE done INT DEFAULT 0;  -- 游标的跳出标识
        ...
        OPEN curClassCourse;  
	        class_loop: LOOP  
		        ...
		        SELECT COUNT(*) INTO @sign FROM admin_performance_teacher a WHERE a.`CLASS_ID`=v_class_id;
		        IF @sign>0 THEN
				DELETE FROM admin_performance_teacher WHERE class_id=v_class_id;
		        END IF;
		        
		        SET tmp_sql_columns = CONCAT(tmp_sql_columns,"class_id,course_id,course_name,grade_id,subject_id,course_time");-- 拼接列sql
		        SET tmp_sql_values = CONCAT(tmp_sql_values,v_class_id,",",v_course_id,",'",v_course_name,"',",v_grade_id,",",v_subject_id,",'",v_course_time,"'");  -- 拼接值sql
			...
			AND ta.`user_type_id`=v_class_id
			AND ta.`e_time`<NOW();
			...
			-- 查询专题下资源学习任务
			SELECT CONCAT(COUNT(t.`TASK_ID`),"|",ROUND(SUM((CASE ta.user_type WHEN 0 THEN (SELECT COUNT(user_id) FROM tp_task_performance tp,tp_task_info t  WHERE tp.creteria_type=t.criteria AND tp.task_id=t.task_id AND tp.task_id=ta.task_id AND tp.user_id IN(SELECT jc.user_id FROM  j_class_user jc LEFT JOIN user_info u ON jc.user_id=u.ref WHERE jc.class_id=ta.user_type_id AND u.state_id=0))
					   WHEN 2 THEN (SELECT COUNT(user_id) FROM tp_task_performance tp,tp_task_info t  WHERE tp.creteria_type=t.criteria AND tp.task_id=t.task_id AND tp.task_id=ta.task_id AND tp.user_id IN(SELECT u.ref FROM user_info u LEFT JOIN tp_j_group_student ts ON u.user_id=ts.user_id WHERE ts.group_id=ta.user_type_id AND u.state_id=0))
			END))/SUM((CASE ta.user_type WHEN 0 THEN (SELECT COUNT(*) FROM j_class_user jc LEFT JOIN user_info u ON jc.user_id=u.ref WHERE jc.class_id=ta.user_type_id AND jc.relation_type="学生" AND u.state_id=0)
				  WHEN 2 THEN (SELECT COUNT(*) FROM tp_j_group_student ts LEFT JOIN user_info u ON ts.user_id=u.user_id WHERE ts.group_id=ta.user_type_id AND u.state_id=0)
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_resource_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_interactive_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_ques_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_coilingtest_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_selftest_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_micro_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_live_task,"0|0.00"),"'");
			...
			SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_general_task,"0|0.00"),"'");
	...
	BEGIN
		...
		DECLARE done INT DEFAULT 0;  -- 游标的跳出标识
		DECLARE curClassCourse CURSOR FOR SELECT c.class_id,tj.`grade_id`,tj.`subject_id`
							FROM class_info c,tp_j_course_class tj,tp_course_info tc
							WHERE c.class_id = tj.class_id
							AND tj.course_id = tc.course_id
							AND tc.`LOCAL_STATUS`=1
							GROUP BY c.`CLASS_ID`,tj.`grade_id`,tj.`subject_id`; 
		DECLARE CONTINUE HANDLER FOR NOT FOUND SET done=1;
		
		OPEN curClassCourse;  
			class_loop: LOOP  
				FETCH curClassCourse INTO v_class_id,v_grade_id,v_subject_id;
				-- 初始化拼接sql
				SET tmp_sql = '';
				SET tmp_sql_columns = '';
				SET tmp_sql_values = '';
				-- 首先查询当前班级，当前专题，有没有记录，如果有，删除，再添加
				SELECT COUNT(*) INTO @sign FROM admin_performance_teacher a WHERE a.`CLASS_ID`=v_class_id AND a.`COURSE_NAME`='总数' AND a.grade_id=v_grade_id AND a.subject_id=v_subject_id;
				IF @sign>0 THEN
					DELETE FROM admin_performance_teacher WHERE class_id=v_class_id  AND grade_id=v_grade_id AND subject_id=v_subject_id;
				END IF;
				
				SET tmp_sql_columns = CONCAT(tmp_sql_columns,"class_id,course_id,course_name,grade_id,subject_id");-- 拼接列sql
				SET tmp_sql_values = CONCAT(tmp_sql_values,v_class_id,",-1,'总数',",v_grade_id,",",v_subject_id);  -- 拼接值sql
			    
				-- 查询专题下任务 
				SELECT COUNT(DISTINCT ta.task_id) INTO v_task_num
				FROM tp_task_info ta,tp_task_allot_info tl,tp_j_course_class tj,tp_course_info tc
				WHERE ta.`TASK_ID`=tl.`task_id`
				AND tj.`course_id`=tc.`COURSE_ID`
				AND ta.`COURSE_ID`=tc.`COURSE_ID`
				AND tc.`LOCAL_STATUS`=1
				AND tj.`grade_id`=v_grade_id
				AND tj.`subject_id`=v_subject_id
				AND (tl.`user_type_id`=v_class_id OR tl.`user_type_id` IN(SELECT group_id FROM tp_group_info WHERE class_id=v_class_id));
				SET tmp_sql_columns = CONCAT(tmp_sql_columns,",task_num");
				SET tmp_sql_values = CONCAT(tmp_sql_values,",",IFNULL(v_task_num,0));
				...
				END))/SUM((CASE ta.user_type WHEN 0 THEN (SELECT COUNT(*) FROM j_class_user jc LEFT JOIN user_info u ON jc.user_id=u.ref WHERE jc.class_id=ta.user_type_id AND jc.relation_type="学生" AND u.state_id=0)
					  WHEN 2 THEN (SELECT COUNT(*) FROM tp_j_group_student ts LEFT JOIN user_info u ON ts.user_id=u.user_id WHERE ts.group_id=ta.user_type_id AND u.state_id=0)
				END))*100,2),"%") INTO v_complete_rate			
				FROM tp_task_allot_info ta,tp_task_info t,tp_j_course_class tj,tp_course_info tc
				WHERE ta.`task_id`=t.`TASK_ID`
				AND tj.`course_id`=tc.`COURSE_ID`
				AND ta.`COURSE_ID`=tc.`COURSE_ID`
				AND tc.`LOCAL_STATUS`=1
				...
				-- 查询专题评价人数，和平均分
				SELECT CONCAT(ROUND(AVG(s.score),2),"|(",COUNT(s.score_user_id),"人)") INTO v_evaluation
				FROM score_info s,j_class_user ju,user_info u,tp_j_course_class tj,tp_course_info tc
				WHERE s.score_user_id = u.`USER_ID`
				AND s.score_object_id=tj.course_id
				AND tj.course_id=tc.course_id
				AND tc.`LOCAL_STATUS`=1
				AND tj.`grade_id`=v_grade_id
				AND tj.`subject_id`=v_subject_id
				...
				AND ju.`CLASS_ID`=v_class_id;
				SET tmp_sql_columns = CONCAT(tmp_sql_columns,",evaluation");
				SET tmp_sql_values = CONCAT(tmp_sql_values,",'",IFNULL(v_evaluation,"0.00(0人)"),"'");
