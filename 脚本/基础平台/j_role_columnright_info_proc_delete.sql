...
DROP PROCEDURE IF EXISTS `j_role_columnright_info_proc_delete`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `j_role_columnright_info_proc_delete`(
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql="delete from j_role_columnright_info where 1=1";
	...
	IF p_role_columnright_id IS NOT NULL THEN
		SET tmp_sql=CONCAT(tmp_sql," and ROLE_COLUMNRIGHT_ID=",p_role_columnright_id);
	END IF;
