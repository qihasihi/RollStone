...
DROP PROCEDURE IF EXISTS `tp_get_task_by_stu_union`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_get_task_by_stu_union`(
					  p_course_id	BIGINT,
				          p_user_id INT,
				          p_task_id BIGINT,
...
FROM (SELECT 1 unionflag,
 ...
 tp_task_performance p,tp_task_allot_info tta,j_class_user cu,user_info u WHERE u.ref=cu.user_id AND u.state_id=0 AND p.user_id=u.ref AND  cu.relation_type='学生' AND cu.class_id=tta.user_type_id AND tta.user_type_id=ta.`user_type_id`  AND tta.task_id=p.task_id AND p.task_id=t.`TASK_ID` and p.creteria_type=t.criteria) 
   WHEN 2 THEN (
	...
	AND tta.task_id=p.task_id AND p.task_id=t.`TASK_ID` AND  p.creteria_type=t.criteria AND tta.user_type_id=ta.`user_type_id`
...
WHERE 1=1 ");
         ...
         /* AND NOT EXISTS(SELECT
                         ref
                       FROM tp_operate_info
                       WHERE target_id = t.task_id
	...
	SET tmp_sql = CONCAT(tmp_sql," union all ","SELECT
  t.*
FROM (SELECT 0 unionflag,
        t.*,tc.course_name,
        ta.b_time,
        ta.e_time,0 total_count,0 stucount,
 ta.user_type_id,
 (SELECT COUNT(*) FROM tp_task_performance p WHERE p.course_id=t.course_id AND p.task_id=t.task_id AND p.creteria_type=t.criteria  AND p.USER_ID=(SELECT ref FROM user_info WHERE user_id=",p_user_id,"))iscomplete,
 (CASE t.task_type 
	WHEN 1 AND t.remote_type IS NOT NULL  THEN  resource_name
 ...
 (SELECT q.question_type FROM question_info q WHERE q.question_id=t.task_value_id)question_type   
      FROM tp_task_info t,tp_task_performance tp,tp_course_info tc,
      tp_task_allot_info ta,
      (SELECT ta.ref
FROM
  tp_task_allot_info ta 
WHERE ta.user_type=2 and  ta.course_id=",p_course_id,"  and NOT EXISTS 
  (
  SELECT 
    g.GROUP_ID 
  FROM
  ...
  WHERE u.user_id = gs.user_id 
    ...
    AND gs.user_id = ",p_user_id,")) tref
      WHERE t.`TASK_ID`=ta.task_id and t.course_id=tc.course_id and ta.ref=tref.ref and tp.task_id=t.task_id and tp.user_id=(select ref from user_info where user_id=",p_user_id,")
          AND t.status = 1) t
WHERE 1=1 ");
/*
SELECT 
...
*/
	...
	SET tmp_sql=CONCAT("select a.* from (",tmp_sql," )a group by task_id,task_value_id order by unionflag desc,order_idx ");
