...
DROP PROCEDURE IF EXISTS `tp_group_student_proc_add`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_group_student_proc_add`(
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql="INSERT INTO tp_group_student (";
	
	IF p_groupid IS NOT NULL THEN
		SET tmp_column_sql=CONCAT(tmp_column_sql,"GROUP_ID,");
		SET tmp_value_sql=CONCAT(tmp_value_sql,"'",p_groupid,"',");
	END IF;
