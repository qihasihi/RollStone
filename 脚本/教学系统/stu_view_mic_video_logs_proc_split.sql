...
DROP PROCEDURE IF EXISTS `stu_view_mic_video_logs_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `stu_view_mic_video_logs_proc_split`(
				          p_ref INT,
				          p_user_id INT,
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'stu_view_mic_video_logs u'; 
