...
DROP PROCEDURE IF EXISTS `tp_resource_rank_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_resource_rank_proc_split`(
				          p_ref INT,
...
BEGIN
	...
	DECLARE tmp_search_column VARCHAR(2000) DEFAULT ' u.*,r.file_name,r.file_size ';  
	DECLARE tmp_search_condition VARCHAR(2000) DEFAULT ' 1=1 and  u.res_id=r.res_id ';  
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT ' tp_resource_rank u,rs_resource_file_info r '; 
