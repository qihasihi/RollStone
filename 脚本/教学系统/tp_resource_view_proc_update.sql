...
DROP PROCEDURE IF EXISTS `tp_resource_view_proc_update`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_resource_view_proc_update`(
				          p_user_id INT,
				          p_ref INT,
				          p_course_id INT,
				          p_res_detail_id INT,
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql ='UPDATE tp_resource_view set m_time=NOW()';
