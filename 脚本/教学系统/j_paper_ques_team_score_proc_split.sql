...
DROP PROCEDURE IF EXISTS `j_paper_ques_team_score_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `j_paper_ques_team_score_proc_split`(
				          p_ref INT,
				             p_paper_id BIGINT,
				            p_ques_ref BIGINT,
				            p_score FLOAT,
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'j_paper_ques_team_score u'; 
	...
	IF p_ques_ref IS NOT NULL THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and u.QUES_REF=",p_ques_ref);
	END IF;
