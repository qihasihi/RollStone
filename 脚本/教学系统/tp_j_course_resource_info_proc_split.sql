...
CREATE DEFINER=`mytest`@`%` PROCEDURE `tp_j_course_resource_info_proc_split`(
					  ...
					  p_has_paper INT,
					  p_vote_uid INT,
					  p_suffixname VARCHAR(50),
					  p_filter_resid BIGINT,
...
BEGIN
	...
	DECLARE tmp_search_column VARCHAR(3000) DEFAULT ' u.ref,u.c_time ctime,u.operate_time,tc.course_name,tc.quote_id,u.course_id,u.resource_type ,r.*,ifnull(ifnull(t.teacher_name,s.stu_name),ui.user_name)real_name,
	(SELECT COUNT(*) FROM tp_task_allot_info ta,tp_task_info t WHERE ta.task_id=t.task_id AND t.course_id=u.course_id AND t.task_value_id=u.res_id)task_flag,
	(SELECT COUNT(*) FROM j_mic_video_paper WHERE mic_video_id=u.res_id)has_paper ';  
	...
	IF p_sel_date_type IS NOT NULL AND p_sel_date_type=1 AND  p_localstatus=2 THEN
	...
	IF p_taskflag IS NOT NULL  THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and  not exists (SELECT 1 FROM tp_task_allot_info ta,tp_task_info t WHERE ta.task_id=t.task_id AND t.course_id=u.course_id AND t.task_value_id=u.res_id ");
		IF p_filter_resid IS NOT NULL  THEN
			SET tmp_search_condition=CONCAT(tmp_search_condition," and u.res_id <>",p_filter_resid," ");
		END IF;
	...
	IF p_suffixname IS NOT NULL THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and r.FILE_SUFFIXNAME='",p_suffixname,"'");
	END IF;
