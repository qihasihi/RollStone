...
DROP PROCEDURE IF EXISTS `teach_version_info_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `teach_version_info_proc_split`(
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'teach_version_info u'; 
	
	IF p_version_name IS NOT NULL THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and u.VERSION_NAME='",p_version_name,"'");
	END IF;
