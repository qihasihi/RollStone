...
BEGIN
 ...
 (SELECT dc_school_id FROM tp_course_info tc,tp_task_info t WHERE tc.course_id=t.course_id AND t.task_id=u.task_id)dc_school_id';  
	DECLARE tmp_search_condition VARCHAR(2000) DEFAULT ' 1=1 AND CASE u.user_type WHEN 2 THEN  (SELECT 1 FROM tp_group_info g,class_info c WHERE  g.class_id=c.class_id AND u.user_type=2 AND u.user_type_id=g.`GROUP_ID`)
ELSE (SELECT 1 FROM class_info WHERE class_id=u.user_type_id AND u.user_type=0)  END  ';  
