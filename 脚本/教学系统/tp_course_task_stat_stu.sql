...
DROP PROCEDURE IF EXISTS `tp_course_task_stat_stu`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_course_task_stat_stu`(p_course_id BIGINT,p_subject_id INT,p_class_id INT,p_sel_type INT,p_group_id BIGINT)
BEGIN
	...
	ELSEIF p_sel_type=2 THEN 
		SET tmp_sql =CONCAT(tmp_sql," AND  (ta.`user_type_id` =",p_class_id," or ta.user_type_id=",p_group_id,") ");
	END IF;
