...
DROP PROCEDURE IF EXISTS `tp_get_add_relate_paper_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_get_add_relate_paper_proc_split`(
					  p_ref INT,
				          p_paper_id BIGINT,
				          p_course_id BIGINT,
...
BEGIN
	...
	DECLARE tmp_search_column VARCHAR(4000) DEFAULT ' distinct u.*,p.paper_name,p.score,p.paper_type,(select count(*) from tp_task_info t where t.task_value_id=u.paper_id and t.course_id=u.course_id)task_flag,
 ...
 AND p.paper_id=u.paper_id AND q.question_type <3)subjective2,
	...
	DECLARE tmp_search_condition VARCHAR(4000) DEFAULT ' 1=1 and u.paper_id=p.paper_id and u.course_id=c.course_id and c.course_id=tcm.course_id and tm.material_id=tcm.teaching_material_id and  g.grade_id=tm.grade_id 
	 and not exists (SELECT 1 FROM j_paper_question pq,question_info q WHERE q.`question_id`=pq.`question_id` AND pq.`paper_id`=u.`paper_id` AND q.`question_type`=5)
	';
	/*and not exists (SELECT 1 FROM j_mic_video_paper vp where vp.paper_id=u.paper_id)*/  
	DECLARE tmp_tbl_name VARCHAR(8000) DEFAULT ' '; 
	
	IF p_course_id IS NOT NULL AND p_mic_video_id IS NOT NULL THEN		
		SET tmp_tbl_name=CONCAT(tmp_tbl_name," (SELECT u.`course_id`,u.`paper_id`,u.`c_time`,u.local_status,null mic_video_id FROM tp_j_course_paper u,paper_info p
		WHERE u.`paper_id`=p.`paper_id`
		AND u.`course_id`=",p_course_id,"  
		 UNION SELECT r.course_id,p.`paper_id`,NULL,1,v.mic_video_id FROM paper_info p,j_mic_video_paper v,tp_j_course_resource_info r 
		 WHERE p.`paper_id`=v.paper_id AND r.res_id=v.mic_video_id AND v.mic_video_id=",p_mic_video_id," AND r.course_id= ",p_course_id,")
		 u,paper_info p,tp_course_info c,tp_j_course_teaching_material tcm,teaching_material_info tm,grade_info g");
	ELSE
		SET tmp_search_condition=CONCAT(tmp_search_condition," and not exists (SELECT 1 FROM j_mic_video_paper vp where vp.paper_id=u.paper_id) ");
		SET tmp_tbl_name=CONCAT(tmp_tbl_name," tp_j_course_paper u,paper_info p,tp_course_info c,tp_j_course_teaching_material tcm,teaching_material_info tm,grade_info g");
