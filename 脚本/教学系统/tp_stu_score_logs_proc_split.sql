...
DROP PROCEDURE IF EXISTS `tp_stu_score_logs_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `tp_stu_score_logs_proc_split`(
				          p_task_id BIGINT,
				          p_ref BIGINT,
				          p_user_id BIGINT,
				          p_class_id BIGINT,
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'tp_stu_score_logs u'; 
