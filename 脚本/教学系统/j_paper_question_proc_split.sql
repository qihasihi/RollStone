...
DROP PROCEDURE IF EXISTS `j_paper_question_proc_split`$$

CREATE DEFINER=`mytest`@`%` PROCEDURE `j_paper_question_proc_split`(
					  p_ref INT,
				          p_paper_id BIGINT,
...
BEGIN
	...
	SET tmp_sql=CONCAT(' SELECT pq.ref,pq.score,pq.order_idx,pq.paper_id,q.*,p.paper_name,p.score pscore,p.paper_type FROM (
		',tmp_sql,'
		) pq,question_info q,paper_info p  WHERE q.question_id=pq.question_id AND p.paper_id=pq.paper_id and q.question_type<7
	');
