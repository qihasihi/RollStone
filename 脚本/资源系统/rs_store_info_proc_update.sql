...
DROP PROCEDURE IF EXISTS `rs_store_info_proc_update`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_store_info_proc_update`(
				          p_user_id INT,
				          p_ref VARCHAR(1000),
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql ='UPDATE rs_store_info set m_time=NOW()';
