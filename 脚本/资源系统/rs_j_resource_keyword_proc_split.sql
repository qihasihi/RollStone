...
DROP PROCEDURE IF EXISTS `rs_j_resource_keyword_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_j_resource_keyword_proc_split`(
				          p_ref INT,
				          p_res_id INT,
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'rs_j_resource_keyword u'; 
	...
	IF p_keyword IS NOT NULL THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and u.KEYWORD='",p_keyword,"'");
	END IF;
