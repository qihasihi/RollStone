...
DROP PROCEDURE IF EXISTS `rs_resource_proc_excellent_resource`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_resource_proc_excellent_resource`(
				       p_res_id VARCHAR(1000),
				          ...
				          p_grade INT,
				          p_subject INT,
				          p_file_type VARCHAR(1000),
				          p_res_type VARCHAR(1000),
...
BEGIN
	...
	IF p_res_name IS NOT NULL THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and r.RES_NAME='",p_res_name,"'");
	END IF;
	...
	IF p_user_name IS NOT NULL THEN
		SET tmp_search_condition=CONCAT(tmp_search_condition," and r.USER_NAME='",p_user_name,"'");
	END IF;
	...
	IF p_type IS NOT NULL THEN
		...
		IF type_num>1 THEN
			SET tmp_search_condition=CONCAT(tmp_search_condition," AND r.FILE_SUFFIXNAME ");
		END IF;
