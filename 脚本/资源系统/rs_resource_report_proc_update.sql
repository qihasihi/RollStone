...
DROP PROCEDURE IF EXISTS `rs_resource_report_proc_update`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_resource_report_proc_update`(
				          p_ref INT,
				          p_user_id INT,
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql ='UPDATE rs_resource_report set m_time=NOW()';
