...
DROP PROCEDURE IF EXISTS `rs_share_info_proc_add`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_share_info_proc_add`(
				            p_user_id  int,
				            p_ref VARCHAR(1000),
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql="INSERT INTO rs_share_info (";
	...
	IF p_state IS NOT NULL THEN
		SET tmp_column_sql=CONCAT(tmp_column_sql,"STATE,");
		SET tmp_value_sql=CONCAT(tmp_value_sql,p_state,",");
	END IF;
