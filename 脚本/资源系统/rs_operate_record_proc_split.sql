...
DROP PROCEDURE IF EXISTS `rs_operate_record_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_operate_record_proc_split`(
				          p_operate_type INT,
				          p_user_id INT,
				          p_res_id BIGINT,
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'rs_operate_record u'; 
