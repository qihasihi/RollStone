...
DROP PROCEDURE IF EXISTS `rs_access_info_proc_update`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_access_info_proc_update`(
				          p_user_id INT,
				          p_enable INT,
				          p_ref VARCHAR(1000),
				          p_res_id BIGINT,
...
BEGIN
	...
	SET affect_row = 0;
	SET tmp_sql ='UPDATE rs_access_info set m_time=NOW()';
