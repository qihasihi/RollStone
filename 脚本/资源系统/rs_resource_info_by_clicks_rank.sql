...
DROP PROCEDURE IF EXISTS `rs_resource_info_by_clicks_rank`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_resource_info_by_clicks_rank`(
...
BEGIN
	...
	SET tmp_sql=CONCAT('
SELECT tmp.viewCount currentClicks,rs.*,IFNULL(sub.SUBJECT_NAME,"--") SUBJECT_NAME,
		IFNULL(g.GRADE_NAME,"--") GRADE_NAME,
		IFNULL(d1.DICTIONARY_NAME,"--") RES_TYPE_NAME,
		IFNULL(d2.DICTIONARY_NAME,"--") FILE_TYPE_NAME
		 FROM (
SELECT t1.res_id,t1.viewCount,t2.grade,t2.subject FROM (
					SELECT res_id,SUM(clicks) viewCount FROM rs_hot_rank WHERE TYPE=',p_type,'
					GROUP BY res_id 
				) t1
				 INNER JOIN 
				(			
	...
	IF p_sort_column IS NULL OR length(p_sort_column)<1 THEN
	   SET tmp_sort=' t1.viewCount desc ';
	   else
