...
DROP PROCEDURE IF EXISTS `rs_share_info_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `rs_share_info_proc_split`(
				          p_user_id INT,
				          p_ref VARCHAR(1000),
...
BEGIN
	...
	DECLARE tmp_tbl_name VARCHAR(2000) DEFAULT 'rs_share_info u'; 
