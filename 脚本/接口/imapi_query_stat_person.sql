...
SELECT ci.user_id,IFNULL (SUM(course_total_score),0)course_total_score,
            IFNULL (SUM(attendance_num),0) attendance_num,IFNULL (SUM(task_score),0) task_score,ci.CLASS_ID,(SELECT ti.group_name FROM tp_group_info ti ,tp_j_group_student ts WHERE ts.group_id=ti.group_id AND ti.group_id=ci.group_id AND ts.user_id=ci.user_id AND ti.subject_id=ci.subject_id)group_name FROM 
