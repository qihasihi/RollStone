...
DROP PROCEDURE IF EXISTS `imapi_task_notcomplete_proc_split`$$

CREATE DEFINER=`schu`@`%` PROCEDURE `imapi_task_notcomplete_proc_split`(
				          p_task_id BIGINT,
				          p_flag INT,
				          p_class_id INT,
				          p_user_id INT,
			...
			class_taskloop:LOOP FETCH class_cursor INTO tmp_user_type,tmp_user_type_id;
				...
				END IF;
			       ...
			       IF tmp_user_type =0 THEN
					SET tmp_tbl=CONCAT(tmp_tbl," union all SELECT u.user_id,concat(c.class_grade,c.class_name)taskobjname,c.class_id FROM j_class_user cu,class_info c,user_info u WHERE cu.class_id=",tmp_user_type_id," and cu.relation_type='Ñ§Éú'  and cu.class_id=c.class_id and cu.user_id = u.ref");
