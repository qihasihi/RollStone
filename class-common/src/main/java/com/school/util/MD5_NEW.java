...
/**
 * <p>Title: MD5加密算法</p>
 * <p>Description: new system of ett,verison 2.0</p>
...
// This class computes MD5_NEW hashes.
...
// $Id: MD5_NEW.java 20876 2012-12-03 06:21:54Z liuyw $
//
// To compute the message digest of a chunk of bytes, create an
// MD5_NEW object 'md5', call md5.update() as needed on buffers full
...
/* This code has been heavily hacked by Tatu Ylonen <ylo@cs.hut.fi> to
   ...
   type. */
/*
 * This code implements the MD5_NEW message-digest algorithm.
 ...
 * MD5_NEW ("a") = 0cc175b9c0f1b6a831c399e269772661
 * MD5_NEW ("abc") = 900150983cd24fb0d6963f7d28e17f72
 * MD5_NEW ("message digest") = f96b697d7cb7938d525a2f31aaf161d0
 * MD5_NEW ("abcdefghijklmnopqrstuvwxyz") = c3fcd3d76192e4007dfb496cca67e13b
 * MD5_NEW ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789") = d174ab98d277d9f5a5611c2c9f419d9f
 * MD5_NEW ("12345678901234567890123456789012345678901234567890123456789012345678901234567890") = 57edf4a22be3c955ac49da2e2107b67a
 */
...
public class MD5_NEW {
    ...
    private void update(byte[] newbuf) {
        update(newbuf, 0, newbuf.length);
    }

    private void update(byte[] newbuf, int length) {
        update(newbuf, 0, length);
    }

    private void update(byte[] newbuf, int bufstart, int buflen) {
        int t;
        int len = buflen;
     ...
     * @return 加密后的字符串
     */
    public static String getMD5Result(String strIn)
    {
        ...
        try {
            if (strIn == null)
                ...
                return getMD5Result(strIn.getBytes("utf-8"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return 加密后的字符串
     */
    public static String getMD5ResultImCode(String strIn)
    {
        String code="im_api_code";
        try {
            if (strIn == null)
                return null;
            else
     ...
     * @return 加密后的字符串
     */
    public static String getMD5ResultEttCode(String strIn)
    {
        String code="ett_api_code";
        try {
            if (strIn == null)
                return null;
            else
     ...
     * @return 加密后的字符串
     */
    public static String getMD5Result(byte[] b)
    {
        byte buf[] = new byte[128];
        int rc;
        MD5_NEW md = new MD5_NEW();
    ...
    /**
     * MD5验证文件
     * @param file 被验证File对象
     * @return 加密字符串
     * @throws java.io.FileNotFoundException 文件不存在
     * @throws java.io.IOException IO错误
     */
    public static String getMD5Result(File file) throws FileNotFoundException,IOException
    {
        ...
        while((tempA=fileIn.read(tempB))!=-1)
        {
            outStr.write(tempB,0,tempA);
        }
        fileIn.close();
        outStr.close();
        return getMD5Result(outStr.toByteArray());
    }
