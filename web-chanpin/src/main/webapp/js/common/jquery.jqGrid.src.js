...
/**
 * @license jqGrid 4.4.0 - jQuery Grid Copyright (c) 2008, Tony Tomov,
 *          tony@trirand.com Dual licensed under the MIT and GPL licenses
 *          http://www.opensource.org/licenses/mit-license.php
 *          http://www.gnu.org/licenses/gpl-2.0.html Date: 2012-06-14
 */
// jsHint options
...
(function ($){
	...
	$.extend($.tableGrid,{		
	    init:function(obj,settings){		
			...
			this.settings.obj=obj;
			this.settings.defaultJsonReader={root:'objList',page:'presult.pageNo',total:'presult.recTotal',records:'presult.recTotal',sort:'orderBy',order:'presult.sort',repeatitems:false};
			this.settings.defaultPrmNames={sort:'pageResult.orderBy',order:'pageResult.sort',page:'pageResult.pageNo',rows:'pageResult.pageSize',oper:"m"};
			this.initSettings();
			this.settings.jsonReader=this.filterObj(this.settings.defaultJsonReader, this.settings.jsonReader);
			this.settings.prmNames=this.filterObj(this.settings.defaultPrmNames, this.settings.prmNames);
			this.initPage();
		},
		initPage:function(){
			$(this.settings.obj).jqGrid({
					url:this.settings.url,
					datatype:'json',  				
					colNames:this.settings.colNames,
					colModel:this.settings.colModel,
					rowNum:this.settings.rowNum,
					rowList:this.settings.rowList,
					pager:this.settings.pager,
					mtype:this.settings.mtype,
					prmNames:this.settings.prmNames,
					jsonReader:this.settings.jsonReader,
					viewrecords:this.settings.viewrecords,
					multiselect:this.settings.multiselect,			
					width: this.settings.width,
					height: this.settings.height,   
					caption:this.settings.title,
					editurl:this.settings.editurl,
					onSelectRow:this.settings.onSelectRow,
					beforeSelectRow: this.settings.beforeSelectRow,
					/**
					 * 
					 */
					onSortCol:  this.settings.onSortCol,
					/**
					 * 双击行的时候 function
					 */
					ondblClickRow:  this.settings.ondblClickRow,
					/**
					 * 
					 */
					onRightClickRow:  this.settings.onRightClickRow,
					/**
					 * 
					 */
					onPaging:  this.settings.onPaging,
					/**
					 * 全部数据选中的时候
					 */
					onSelectAll:  this.settings.onSelectAll,
					/**
					 * 加载数据成功
					 */
					loadComplete:  this.settings.onSelectAll,
					/**
					 * 窗体成功
					 */
					gridComplete:  this.settings.gridComplete,
					/**
					 * 加载错误
					 */
					loadError:  this.settings.loadError,
					/**
					 * 
					 */
					loadBeforeSend:  this.settings.loadBeforeSend,
					/**
					 * 
					 */
					afterInsertRow:  this.settings.afterInsertRow,
					/**
					 * 
					 */
					beforeRequest:  this.settings.beforeRequest,
					/**
					 * 
					 */
					beforeProcessing :  this.settings.beforeProcessing,
					/**
					 * 列表头点击时 function
					 */
					onHeaderClick:  this.settings.onHeaderClick
				});
				var editopeate={};
				if(!this.settings.isAdd)
					editopeate.add=false;
				if(!this.settings.isUpdate)
					editopeate.edit=false;
				if(!this.settings.isDel)
					editopeate.del=false;
			return	$(this.settings.obj).jqGrid('navGrid',this.settings.pager,editopeate);   	
		},filterObj:function(fobj,eobj){
			if(typeof(fobj)!="object")
				fobj={};
			var ekeys=this.getKeys(eobj);
			var evals=this.getValues(eobj);
			for(var i=0;i<ekeys.length;i++){
				eval("(fobj."+ekeys[i]+"='"+evals[i]+"')");
			}
			return fobj;
		},
	    initSettings : function() {
			...
			this.ensureDefault("url", ""); // * 链接地址
			this.ensureDefault("prmNames",this.settings.defaultPrmNames); // ID名称
																																		// *
			this.ensureDefault("jsonReader",
					this.settings.defaultJsonReader); // 名称，声明变量的名称
			this.ensureDefault("colNames", []); // 控件生成在页面ID地址 *
			this.ensureDefault("colModel", []); // 参数
			this.ensureDefault("rowNum", 20);
			this.ensureDefault("rowList", [1,2,3,5,10,15,20,25,30,35,40,45,50]);
			this.ensureDefault("mtype", 'post');		// 执行查询时，执行的方法
			this.ensureDefault("viewrecords", true);// 正在访问中，执行的方法 *
			this.ensureDefault("multiselect",true);  // 操作中执行的方法
			this.ensureDefault("width", 900);// 访问成功后 返回值类型. JSON? *
			this.ensureDefault("height", "auto"); // 页码 *
			this.ensureDefault("title", ""); // 显示数 *
			this.ensureDefault("editurl", ""); // 数据总数量 *
			this.ensureDefault("isAdd", true); // 页数总数量 *
			this.ensureDefault("isUpdate", true); // 排序
			this.ensureDefault("isDel", true); //
			this.ensureDefault("datatype", 'json'); //
			this.ensureDefault("onSelectRow", null); //
			this.ensureDefault("beforeSelectRow", null); //
			this.ensureDefault("onSortCol", null); //
			this.ensureDefault("ondblClickRow", null); //
			this.ensureDefault("onRightClickRow",null); //
			this.ensureDefault("onPaging", null); //
			this.ensureDefault("onSelectAll",null); //
			this.ensureDefault("loadComplete",null); //
			this.ensureDefault("loadError",function(){alert('加载数据错误!');}); //
			this.ensureDefault("loadBeforeSend",null); //
			this.ensureDefault("afterInsertRow",null); //
			this.ensureDefault("beforeRequest",null); //
			this.ensureDefault("beforeProcessing",null); //
			this.ensureDefault("onHeaderClick",null); //
			this.ensureDefault("pager", "");// 操作对象
	...
	// 添加的tableGrid方法
	$.fn.tableGrid=function(settings){
		return $.tableGrid.init(this,settings);	
	}
...
$.extend($.jgrid,{
	...
	getXmlData: function (obj, expr, returnObj) {
		...
		if (m && m[2]) {
			...
			// $(expr, obj).filter(':last'); // we use ':last' to be more
			// compatible with old version of jqGrid
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			pager: "",			// 分页控件所在的位置
			/**
			 * boolean 确定是否显示pager的按钮。 默认值:true 该属性不能修改
			 */
			pgbuttons: true,	
			/**
			 * boolean 确定是否显示允许用户直接输入页码进行跳转的输入框。 默认值:true 该属性不能修改
			 */
			pginput: true,
			/**
			 * array 重要的属性。用来设置列名称。 默认值:empty array 该属性不能修改
			 */
			colModel: [],
			/**
			 * array[]
			 * 本参数用来为grid生成一个可供选择的调整每页显示记录条数的下拉菜单。在grid中选择修改这个值，jqGrid会用新的选择值来修改rowNum参数并传递到服务器端。可以设置的格式如[10,20,30]
			 * 默认值:empty array - [] 该属性不能修改
			 */
			rowList: [],
			/**
			 * array[] 重要的属性。用来设置Grid每一个列的属性，一个列作为一个数组元素设置。Name属性是必须的。
			 * 注意：这里的数组长度（也就是列数）必须和colModel数组的长度一致。更详细的内容，需要单独说明。 默认值:empty
			 * array[] 该属性不能修改
			 */
			colNames: [],
			/**
			 * string 当使用xml或者json数据类型时，设置默认的排序方式。可选值asc或desc. 默认值:asc 该属性可以修改
			 */
			sortorder: "asc",
			/**
			 * string 当使用xml或者json数据类型时，设置默认的排序字段名称。这个参数会被增加到url上传递到服务器端。
			 * 默认值:empty string 该属性可以修改
			 */
			sortname: "",
			/**
			 * string 确定grid中显示的数据源格式。 xml –xml数据; xmlstring – 被当做字符串的xml数据；
			 * json –JSON格式数据 ； jsonstring – 被当做字符串的JSON数据； local – 定义在客户端的数组数据；
			 * javascript –javascript的数据 function – 客户定义函数用来获取数据； 默认值:xml
			 * 该属性可以修改
			 */
			datatype: "xml",
			/**
			 * string 确定请求方式。“POST”或“GET” 默认值:GET 该属性可以修改
			 */
			mtype: "GET",
			/**
			 * boolean 设置jqGrid显示为斑马线样式。就是隔行用不同的背景色区分开。 默认值:false 该属性可以修改. After
			 * reload
			 */
			altRows: false,
			/**
			...
			selarrrow: [],
			/**
			 * array 只读属性。本属性用在Inline Editing和Cell Editing模式下，用来在编辑行或者单元格之前保存数据。
			 * 默认值:empty array 该属性不能修改
			 */
			savedRow: [],
			/**
			...
			shrinkToFit: true,
			/**
			 * array 对xml的数据格式的说明。参考jsonReader的描述。 默认值：无 该属性不能修改
			 * 
			 */
			xmlReader: {},
			/**
			 ...
			 * 
			 * jsonReader:{ root:”datas”, page:”currentPage” } 默认值:无 该属性不能修改
			 */
			jsonReader: {},
			/**
			 * boolean 设置是否能使用subgrid。 默认值:false 该属性不能修改
			 */
			subGrid: false,
			/**
			 * array-[]
			...
			reccount: 0,
			/**
			 * integer 只读属性，指定请求总共可以返回多少页。 默认值:0 该属性不能修改
			 */
			lastpage: 0,
			/**
			 * integer 只读属性。指定了最后排序列。从0开始计数。 默认值:0 该属性不能修改
			 */
			lastsort: 0,
			/**
			 * string 只读属性。最后选中行的id。如果重新排序或者进行了导航，这个值为null。（也就是没有选中任何行的意思！）
			...
			onHeaderClick: null,
			/**
			 * boolean 确定是否显示总的记录条数。 默认值:false 该属性不能修改
			 */
			viewrecords: false,
			/**
			 * boolean
			...
			loadonce: false,
			/**
			 * boolean 定义grid中的数据行是否可以被多选。会在grid的最左边增加一列checkbox的选择框。 默认值:false
			 * 该属性不能修改. see HOWTO
			 */
			multiselect: false,
			/**
			 * string 只有当multiselect选项设置为true的时候才有意义。定义使用哪一个键来做多选。可选值:
			 * shiftKey/altKey/ctrlKey 默认值:empty string 该属性可以修改
			 */
			multikey: false,
			/**
			 * string 设置编辑的URL地址。 默认值:null 该属性可以修改
			 */
			editurl: null,
			/**
			 * 
			...
			caption: "",
			/**
			 * boolean 设置grid的隐藏/显示按钮是否可用。 默认值:true 该属性不能修改
			 */
			hidegrid: true,
			/**
			 * boolean
			 ...
			 * 默认值:empty array 该属性可以修改
			 */
			postData: {},
			/**
			 * array 本属性包含一些从request中获取的自定义的信息。比如不想直接显示在grid中的一些数据。 默认值:empty
			 * array 该属性不能修改
			 */			
			userData: {},
			/**
			 * boolean 开启或者禁用树形grid的模式。 默认值:false 该属性不能修改
			 */
			treeGrid : false,
			/**
			 * string 确定treeGrid的方法。可能是nested或adjacency 默认值:nested 该属性不能修改
			 */
			treeGridModel : 'nested',
			/**
			 * array 扩展grid的colModel且加在colModel定义的后面这意味着从服务器返回的数据必须包含这些字段值。
			 * 默认值：无 该属性不能修改
			 */
			treeReader : {},
			/**
			...
			ExpandColumn: null,
			/**
			 * numeric treeGrid root元素的级别 默认值:0 该属性不能修改
			 */
			tree_root_level : 0,
			
			/**
			 * array 定制传递到服务器上的Post数据的字段名称。默认值是： prmNames:
			 * {page:“page”,rows:“rows”, sort: “sidx”,order: “sord”,
			 * search:“_search”, nd:“nd”, npage:null} 默认的情况下，上传的数据如下url所示：
			 * URL?page=1&rows=10&sidx=myindex&sord=asc
			 * 如果我们设置一下本属性{sort:”mysort”}那么实际调用的URL地址如下：
			 * URL?page=1&rows=10&mysort=myindex&sord=asc 通过这个属性的设置，可以改变字段的名称。
			 * 默认值:none 该属性可以修改
			 */
			...
			forceFit : false,
			/**
			 * string 设置grid的状态。有两个可选值'visible'和'hidden' 默认值:visible 该属性不能修改
			 */
			gridstate : "visible",
			/**
			 * boolean 设置Cell Editing模式是否启用。 默认值:false 该属性可以修改
			 */
			cellEdit: false,
			/**
			 * string 确定Cell Editing模式下，内容保存到远程服务器还是本地数组。'remote'
			 * 或者'clientArray'。 默认值:'remote' 该属性可以修改
			 */
			cellsubmit: "remote",
			/**
			 * 
			...
			nv:0,
			/**
			 * string 本选项控制在一个ajax请求执行过程中做什么。 disable –jqGrid的指示信息不可用。 enable -
			 * 显示 “Loading”信息。 block –
			 * 可以显示“Loading”信息，同时阻止grid的所有操作直到ajax请求完成。注意，这会禁止所有的排序、导航或者任何工具条的操作。
			 * 默认值:enable 该属性可以修改
			 */			
			loadui: "enable",
			/**
			 * array Grid的工具栏。数组中有两个值，第一值指示toolbar是否可用，第二个值定义了toolbar相对于body
			 * Layer的位置，可能的值包括“top”,”bottom”, “both”
			 * 当我们设置值为[true,”both”]，会创建两个工具栏，一个在table的顶部，一个在table的底部。
			 * 顶部的工具栏的div的id等于”t_”+grid的Id。底部的工具栏的div的id等于”tb_”+grid的Id。
			 * 注意：当只创建一个工具栏的时候，不管位置是在顶部还是底部，工具栏的div的id都等于”t_”+grid的Id。
			 * 默认值:[false,''] 该属性不能修改
			 */
			toolbar: [false,""],
			/**
			...
			scroll: false,
			/**
			 * boolean 只有当multiselect =
			 * true时起作用，如果multiselect设置为true，那么点击行任何位置都可以选中行。当multiboxonly设置为ture之后，就只有点击行前面的checkbox才会选中行。如果点击其它没有被选中的行，那么点中的行被选择，其它行取消选择。(Yahoo模式)
			 * 默认值:false 该属性可以修改
			 */
			multiboxonly : false,
			/**
			 * boolean 只有当datatype设置为local才有效。当排序之后，是否重新选择排序之前选中的行(多行)。 默认值:true
			 * 该属性可以修改
			 */
			deselectAfterSort : true,
			/**
			 * boolean
			 * 设置为true，当我们用setSelection选中记录时，grid会自动滚动到选中的行记录，让该行记录可见。假设grid有垂直滚动条，当我们在Form
			 * Editing编辑模式下修改记录，用编辑页的前后导航按钮定位记录的时候，当导航到一个隐藏的行记录的时候，grid会自动滚动以使让选中的行记录变成可见。
			 * 默认值:false 该属性可以修改
			 */
			scrollrows : false,
			/**
			...
			autowidth: false,
			/**
			 * integer 设置垂直滚动条宽度。 默认值:18 该属性不能修改.Method avail.
			 * 
			 */
			scrollOffset :18,
			/**
			 * integer 本选项设置单元格的padding + border宽度. 一般情况下不要去修改。 默认值:5 该属性不能修改
			 */
			cellLayout: 5,
			/**
			 * integer 设置subGrid列的宽度。 默认值:20 该属性不能修改
			 */
			subGridWidth: 20,
			/**
			 * integer 当multiselect为true时设置multiselect列宽度 默认值:20 该属性不能修改
			 */			
			multiselectWidth: 20,
			/**
			 * boolean 在包括jqGrid
			 * v3.4.X之前的版本中，读取大数据集会存在速度问题。主要原因就是每一个单元格都是用了5-6条jQuery调用被插入到grid中。现在我们可以通过设置gridview
			 * 选项为true来解决这个问题。这时候，我们用一个jQuery追加一次插入整行的数据。比以前快了3-5倍。
			 * 当然，使用也有限制，设置为true之后，不能使用treeGrid，subGrid或者afterInsertRow事件。
			 * 默认值:false 该属性可以修改
			 */
			gridview: false,
			/**
			 * integer 如果rownumbers设置为true，本属性用来设置该列的宽度。 默认值:25 该属性不能修改
			 */
			rownumWidth: 25,
			/**
			 * boolean
			...
			pagerpos: 'center',
			/**
			 * string 确定记录条数信息在pager上面的位置。可能的值是left, center, right 默认值:right
			 * 该属性不能修改
			 */
			recordpos: 'right',
			/**
			 ...
			 * The rules are as follow: If the userData array contain name which
			 * is equal to those of colModel then the value is placed in that
			 * column.If there are 该属性不能修改such values 该属性不能修改thing is palced.
			 * 默认值:false 该属性可以修改
			 */
			userDataOnFooter : false,
			/**
			 * boolean 设置为false，在grid的数据行中鼠标悬浮被禁用。 默认值:false 该属性可以修改
			 */
			hoverrows : true,
			/**
			 * string 用来设置jqGrid的交替行的class。只有设置了altRows为true才有效。
			 * 默认值:ui-priority-secondary 该属性可以修改. Requires reload
			 * 
			 */
			altclass : 'ui-priority-secondary',
			/**
			 * array 本参数主要用来定义出现在标题附近的排序图标的不同的外观和行为。
			 * 参数数组的组成如下：[false,'vertical',true].
			 * 第一个参数确定，如果所有的列排序属性都是true，是否所有的图标都能同时看到。默认值false表示只有最后排序列的图标可见。
			 * 第二个参数确定图标的位置。可能的值是vertical 表示排序图标一个在另一个的上面。'horizontal'
			 * 表示水平放置两个图标。
			 * 第三个参数确定鼠标点击事件功能。设置为true，表示，表示点击列标题可排序；设置为false表示只能点击图标才能排序。
			 * 重要提示：如果设置第三个参数为false，那么必须要设置第一个参数为true，否则，排序功能可能丢失。 默认值：无 该属性不能修改
			 */
			viewsortcols : [false,'vertical',true],
			/**
			 * string 为列指定一个调整宽度时候的现实风格class。 默认值:empty string 该属性不能修改
			 */
			resizeclass : '',
			/**
			 * boolean 设置为true，会对从服务器返回的数据以及编辑模式下提交的数据进行HTML编码。比如，把小于符号转换成&lt;
			 * 默认值:false 该属性可以修改
			 */
			autoencode : false,
			/**
			 ...
			 * 默认值:empty object 该属性可以修改
			 */
			ajaxGridOptions :{},
			/**
			 * string 确定grid中文本显示的方向。默认ltr表示从左道右。还有一个可选值是rtl表示从右到左。 本选项只在FireFox
			 * 3.x和IE >=6的浏览器有效。当前Safai和Google
			 * Chrome、Opera不完全支持“rtl”。FireFox默认设置是不支持RTL的，但是可以通过修改FireFox的layout.scrollbar.side设置为1来支持。本属性是在jqGrid
			 * v3.6版本中新增加的。 默认值:ltr 该属性不能修改
			 */
			direction : "ltr",
			/**
			 * 
		...
		sortData = function (index, idxcol,reload,sor){
			...
			index = index.substring(5 + ts.p.id.length + 1); // bad to be
																// changed!?!
...
$.jgrid.extend({
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				// follow the original set height to use 16, better scrollbar
				// width detection
 ...
 * 
 * Wildraid wildraid@mail.ru Oleg Kiriljuk oleg.kiriljuk@ok-soft-gmbh.com Dual
...
$.jgrid.extend({
			...
			$.each($t.p.colModel,function(){
				...
				if(this.search){
					...
					case "select":
						...
						if(surl) {
							// data returned should have already constructed
							// html select
		...
		return this.each(function(){
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					// caclulate the number of visible columns from the next
					...
					// The next numberOfColumns headers will be moved in the
					// next row
					// in the current row will be placed the new column header
					// with the titleText.
					...
					$th.before($colHeader); // insert new column header before
											// the current
					$tr.append(th);         // move the current header in the
											// next row

					// set the coumter of headers which will be moved in the
					// next row
	...
	destroyFrozenColumns :  function() {
		return this.each(function() {
			 ...
			 * 
			 * Copyright (c) 2007,2008 Brice Burgess <bhb@iceburg.net> Dual
			 * licensed under the MIT and GPL licenses:
			 * http://www.opensource.org/licenses/mit-license.php
		...
		DateFormat : function (format, date, newformat, opts)  {
			...
			var 
				...
				flags = {
					...
					N: N,
					S: opts.S(j),
					// j < 11 || j > 13 ? ['st', 'nd', 'rd', 'th'][Math.min((j -
					// 1) % 10, 3)] : 'th',
					w: w,
					z: z,
					// Week
...
// Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {		
		...
		mw.appendChild(mc);
		$(mw).prepend(mh);
		if(appendsel===true) { $('body').append(mw); } // append as first child
														// in body -for alert
														// dialog
		...
		$(mw).css(css);
		if(typeof p.jqModal === 'undefined') {p.jqModal = true;} // internal
																	// use
		var coord = {};
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		// if the id is not provided we set it like info_button_+ the index in
		// the array - i.e info_button_0,info_button_1...
		};
							...
							if(a) {
								...
								setTimeout(function(){
									$("option",elem).each(function(i){
										...
										// fix IE8/IE7 problem with selecting of
										// the first item on multiple=true
...
// Date Validation Javascript
	...
	checkValues : function(val, valref,g, customobject, nam) {
		...
		return [true,"",""];
			 ...
			 * jqFilter jQuery jqGrid filter addon. Copyright (c) 2011, Tony
			 * Tomov, tony@trirand.com Dual licensed under the MIT and GPL
			 ...
			 * 
			 * The filter uses JSON entities to hold filter rules and groups.
			 * Here is an example of a filter: { "groupOp": "AND", "groups" : [ {
			 * "groupOp": "OR", "rules": [ { "field": "name", "op": "eq",
			 * "data": "England" }, { "field": "id", "op": "le", "data": "5"} ] } ],
			 * "rules": [ { "field": "name", "op": "eq", "data": "Romania" }, {
...
$.fn.jqFilter = function( arg ) {
	...
	var p = $.extend(true,{
		...
		numopts : ['=','<>', '<', '<=', '>', '>=', 'IS NULL', 'IS NOT NULL', 'IN', 'NOT IN'],
		stropts : ['=', '<>', 'LIKE', 'NOT LIKE', 'LIKE', 'NOT LIKE', 'LIKE', 'NOT LIKE', 'IS NULL', 'IS NOT NULL', 'IN', 'NOT IN'],
	...
	return this.each( function() {
		...
		/*
		 * Redraw the filter every time when new field is added/deleted and
		 * field is changed
		 */
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.bind('change',function() {
				...
				var s ="", so = 0;
				aoprs = [];
				$.each(that.p.ops, function() { aoprs.push(this.operator) });
				for ( i = 0 ; i < op.length; i++) {
					ina = $.inArray(op[i],aoprs);
					if(ina !== -1) {
						if(so===0) {
							rule.op = that.p.ops[ina].operator;
						}
						s += "<option value='"+that.p.ops[ina].operator+"'>"+that.p.ops[ina].description+"</option>";
						so++;
					}
				}
			...
			ruleOperatorSelect.bind('change',function() {
				...
				if (rule.op === "nu" || rule.op === "nn") { // disable for
															// operator "is
															// null" and "is not
															// null"
					rule.data = "";
			...
			for ( i = 0; i < op.length; i++) {
				ina = $.inArray(op[i],aoprs);
				if(ina !== -1) {
					selected = rule.op === that.p.ops[ina].operator ? " selected='selected'" : "";		
					// 自定义修改 that.p.ops[ina].name
					str += "<option value='"+that.p.ops[ina].operator+"'"+selected+">"+that.p.ops[ina].description+"</option>";
				}
			}
...
$.jgrid.extend({
	searchGrid : function (p) {
		p = $.extend({
			...
			sValue:'searchString',
			sOper: 'searchOper',
			sFilter: 'pageResult.objList',
			loadDefaults: true, // this options activates loading of default
								// filters from grid's postData for Multipe
								// Search only.
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal)).html() !== null ) {
				...
				if(!p.columns.length) {
					$.each(columns, function(i,n){
						...
						// find first searchable column and set it if no default
						// filter
						if(!found) {
				...
				if (p.stringResult===undefined) {
					// to provide backward compatibility, inferring stringResult
					// value from multipleSearch
		...
		return this.each(function(){
			...
			function postIt() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					if(postdata[oper] != opers.addoper) {
						...
						// check to see if we have allredy this field in the
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,Status){
							...
							if(Status != "success") {
								...
								// execute aftersubmit with the returned data
								// from server
							...
							if(ret[0] === false) {
								...
								// remove some values if formattaer select or
								// checkbox
								$.each($t.p.colModel, function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal)).html() !== null ) {
				...
				$("#sData", frmtb+"_2").click(function(){
					...
					// ret[2] - the id that will be set if reload after submit
					// false
					getFormData();
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal)).html() !== null ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					var ret=[true,""];onCS = {};
					var postdata = $("#DelData>td","#"+dtbl).text(); // the
																		// pair
																		// is
																		// name=val1,val2,...
						...
						var ajaxOptions = $.extend({
							...
							complete:function(data,Status){
								if(Status != "success") {
									...
									// data is posted successful
									// execute aftersubmit with the returned
									// data from server
...
// Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if( $.type(args[0]) === "object" ) {
			...
			// if (typeof restoreAfterError !== "undefined") {
 ...
 */ 
/**
 * all events and options here are aded anonynous and not in the base grid since
 * the array is to big. Here is the order of execution. From this point we use
 * jQuery isFunction formatCell beforeEditCell, onSelectCell (used only for
 * noneditable cels) afterEditCell, beforeSaveCell, (called before validation of
 * values if any) beforeSubmitCell (if cellsubmit remote (ajax))
 * afterSubmitCell(if cellsubmit remote (ajax)), afterSaveCell, errorCell,
 * serializeCellData - new Options cellsubmit (remote,clientArray) (added in
 * grid options) cellurl ajaxCellOptions
 */
"use strict";
$.jgrid.extend({
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 9)  {
						if(!$t.grid.hDiv.loading ) {
							if (e.shiftKey) {$($t).jqGrid("prevCell",iRow,iCol);} // Shift
																					// TAb
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true) {
							$(r).remove(".ui-subgrid");
						} else if($(r).hasClass('ui-subgrid')) { // incase of
																	// dynamic
																	// deleting
							$(r).hide();
						}
...
$.jgrid.extend({
	...
	groupingGroupBy : function (name, options ) {
		return this.each(function(){
			...
			var i;
			// show previous hidden groups if they are hidden and weren't
			// removed yet
 ...
 * tony@trirand.com http://trirand.com/blog/ Dual licensed under the MIT and GPL
 * licenses: http://www.opensource.org/licenses/mit-license.php
 * http://www.gnu.org/licenses/gpl-2.0.html
 */ 
// jsHint options
            ...
            return this.each(function(){
                ...
                switch (o.imptype){
                    case 'xml':
                        ...
                        break;
                    case 'xmlstring' :
                        // we need to make just the conversion and use the same
						// code as xml
...
/*
 * * jqGrid addons using jQuery UI Author: Mark Williams Dual licensed under the
...
$.jgrid.extend({
    ...
    columnChooser : function(opts) {
        ...
        opts = $.extend({
            ...
            /*
			 * msel is either the name of a ui widget class that extends a
			 * multiselect, or a function that supports creating a multiselect
            ...
            /*
			 * dlog is either the name of a ui widget class that behaves in a
			 * dialog-like way, or a function, that supports creating a dialog
			 * (when passed dlog_opts) or destroying a dialog (when passed the
			 * string "destroy")
			 */
            "dlog" : "dialog",
			"dialog_opts" : {
            ...
            /*
			 * dlog_opts is either an option object to be passed to "dlog", or
			 * (more likely) a function that creates the options object. The
			 * default produces a suitable options object for ui.dialog
			 */
            ...
            /*
			 * Function to get the permutation array, and pass it to the "done"
			 * function
			 */
            "apply_perm" : function() {
            ...
            /*
			 * Function to cleanup the dialog, and select. Also calls the done
			 * function with no permutation (to indicate that the columnChooser
			 * was aborted
			 */
			...
			"drag" : function (opts) {
				return $.extend({
					start : function (ev, ui) {
						...
						// hack
						// drag and drop does not insert tr in table, when the
						// table has no rows
					...
					stop :function(ev,ui) {
						...
						// if we have a empty row inserted from start event try
						// to delete it
						for (var i=0;i<$.data($t,"dnd").connectWith.length;i++){
				...
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						if(opts.beforedrop && $.isFunction(opts.beforedrop) ) {
							// parameters to this callback - event, element,
							// data to be inserted, sender, reciever
							// should return object which will be inserted into
							// the reciever
						...
						if(ui.helper.dropped) {
							...
							// NULL is interpreted as undefined while null as
							// object
	...
	gridResize : function(opts) {
		return this.each(function(){
			...
			if(opts._alsoResize_) {
				...
				opts.alsoResize = eval('('+optstest+')'); // the only way that
															// I found to do
															// this
			} else {
